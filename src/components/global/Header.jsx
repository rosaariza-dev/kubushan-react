...
const Header = () => {
  ...
  const headerOverlayClasses = clsx(
    // Estilos base/mobile
    "fixed inset-0 z-10 bg-black/60 transition-opacity duration-300",
    // Estilos condicionales
    isOpen ? "block" : "hidden"
  );
  ...
  return (
    <header className="font-primary pt-7 pb-3 px-8 max-w-full md:px-16">
      <nav className="flex items-center justify-between">
        ...
        <button
          className="flex cursor-pointer border-none bg-transparent md:hidden"
        ...
        <ul className={menuClasses}>
          <li className="p-3 md:hidden">
            <button
              ...
              className="text-white bg-custom-black border-none p-1 w-7 h-7 rounded-lg cursor-pointer flex items-center justify-center "
              onClick={closeMenu}
            >
              <X />
            </button>
          ...
          {navItems.map(({ title, link }) => (
            <li key={title}>
              <a
                href={link}
                className="flex p-3 w-full rounded-lg hover:bg-custom-gray/20 md:rounded-full md:px-4 md:py-2"
                onClick={closeMenu}
              >
                {title}
              </a>
        ...
        <div
          aria-hidden="true"
          className={headerOverlayClasses}
          onClick={closeMenu}
        ></div>
      </nav>
    </header>
