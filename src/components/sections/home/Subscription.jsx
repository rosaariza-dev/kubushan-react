...
const Subscription = () => {
  ...
  return (
    <section className={subscriptionClasses}>
        ...
        <span className='font-lg font-secondary text-white/80 mb-6 md:text-[28px] md:mb-14'>Recibe lo mejor de Kubushan directo en tu correo.</span>
        <div className='flex items-center flex-col w-full font-secondary gap-6 md:flex-row md:border md:border-white max-w-[700px] md:rounded-[64px] md:p-5'>
            <input className='p-6 w-full text-center text-base rounded-[44px] border border-white outline-none text-white placeholder:text-base placeholder:text-white/50 placeholder:font-secondary placeholder:text-center md:flex-1 md:border-none md:text-[22px] md:text-start md:placeholder:text-[22px] md:placeholder:text-start' type="text" placeholder='Ingresa tu correo electrónico' />
            <button className='flex items-center justify-center bg-custom-black text-white min-w-[230px] p-4 font-secondary font-medium rounded-36 border-none gap-3 cursor-pointer text-base hover:bg-neutral-900 md:p-5 md:text-[22px]'><Send size={24} />Enviar</button>
        </div>
    </section>
  )
}
